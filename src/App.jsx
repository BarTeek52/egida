...
const styles = {
  headerFont: { fontFamily: "'Semplicita Pro', sans-serif" },
  bodyFont: { fontFamily: "'Kiro', sans-serif" }
};
...
export default function App() {
  ...
  // Definicja modułów z kolorami ikonek
  const modules = [
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      ...
      setRecords(data);
    }, (err) => console.error("Błąd bazy:", err));
    return () => unsubscribe();
  }, [user]);

  // --- LOGIKA MODUŁU WYPOWIEDZENIA ---
  ...
  const getInputClass = (fieldName, type = 'text') => {
    ...
    let base = "w-full p-4 rounded-2xl border outline-none transition-all placeholder:text-slate-300 placeholder:text-sm font-medium";
    ...
    const status = isError 
        ...
        : "border-slate-100 bg-white shadow-sm focus:border-blue-400 focus:ring-4 focus:ring-blue-50";
    return `${base} ${status}`;
  };
  ...
  const renderContent = () => {
      ...
      return (
        <div className="p-6 md:p-12 space-y-12 animate-in fade-in duration-500">
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
              <p className="text-[#0067b1] font-bold text-xs uppercase tracking-[0.2em] mb-2">Witamy ponownie</p>
              <h1 className="text-4xl font-black text-slate-900" style={styles.headerFont}>Dzień dobry, Bartek</h1>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[#0067b1] p-8 rounded-[2.5rem] text-white shadow-2xl shadow-blue-200 flex flex-col justify-between overflow-hidden relative group">
                ...
                <div>
                  ...
                  <p className="text-6xl font-black mt-2 leading-none">{records.length}</p>
                </div>
                <div className="mt-8 flex items-center gap-2 text-sm font-bold text-blue-200">
                  <ArrowUpRight size={16} /> Zobacz szczegóły
                </div>
            </div>
            ...
            {modules.filter(m => m.id !== 'dashboard').map((mod) => (
              ...
              >
                ...
                <span className="text-xl font-black text-slate-800" style={styles.headerFont}>{mod.label}</span>
                <p className="text-slate-400 text-sm mt-2 font-medium" style={styles.bodyFont}>{mod.desc}</p>
              </button>
            ))}
          </div>
        </div>
    ...
    if (activeTab === 'wypowiedzenia') {
      return (
        <div className="max-w-4xl mx-auto p-4 md:p-12 space-y-8 pb-20 animate-in slide-in-from-bottom-4 duration-500">
            <header className="flex flex-col sm:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h1 className="text-3xl font-black text-slate-900 tracking-tight" style={styles.headerFont}>Wypowiedzenia</h1>
                      <p className="text-slate-400 font-medium text-sm">Generator dokumentów OC art. 28, 28a, 31</p>
                    </div>
                </div>
                <button onClick={() => setShowResetConfirm(true)} className="px-8 py-3 bg-white text-rose-500 border border-rose-100 rounded-2xl font-bold text-sm hover:bg-rose-50 transition-all shadow-sm">Wyczyść formularz</button>
            </header>

            <div className="space-y-8">
                <section className="bg-[#0067b1] rounded-[2.5rem] p-8 shadow-2xl shadow-blue-100">
                    <label className="block text-xs font-bold text-blue-100 mb-3 uppercase tracking-[0.2em]">Szybkie wyszukiwanie</label>
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="relative flex-1">
                          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-300" size={20} />
                          <input 
                            type="text" 
                            className="w-full pl-12 pr-4 py-5 rounded-2xl bg-white/10 border border-white/20 text-white placeholder:text-blue-200/50 uppercase font-bold text-lg outline-none focus:bg-white/20 transition-all" 
                        ...
                        <button onClick={handleSearch} className="bg-white text-[#0067b1] px-10 py-5 rounded-2xl font-black hover:bg-blue-50 transition-all shadow-xl flex items-center justify-center gap-2 uppercase tracking-widest text-sm">
                            Szukaj w bazie
                        </button>
                    </div>
                </section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-6">
                        <h2 className="font-black text-[#0067b1] uppercase text-xs tracking-[0.2em] flex items-center gap-2">
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <input name="imieNazwisko" placeholder="np. Jan Kowalski" className={getInputClass('imieNazwisko')} value={formData.imieNazwisko} onChange={handleInputChange} />
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Ulica i numer</label>
                                <input name="ulica" placeholder="np. Polna 12/3" className={getInputClass('ulica')} value={formData.ulica} onChange={handleInputChange} />
                            </div>
                            <div className="flex gap-4">
                                <div className="w-1/3">
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Kod</label>
                                    <input name="kodPocztowy" placeholder="00-000" className={getInputClass('kodPocztowy')} value={formData.kodPocztowy} onChange={handleInputChange} maxLength={6} />
                                </div>
                                <div className="flex-1">
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Miejscowość</label>
                                    <input name="miejscowosc" placeholder="np. Szczecin" className={getInputClass('miejscowosc')} value={formData.miejscowosc} onChange={handleInputChange} />
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-6">
                        ...
                        <div className="space-y-5">
                            <div>
                                ...
                                <input name="nrRejestracyjny" placeholder="ABC 12345" className={getInputClass('nrRejestracyjny')} value={formData.nrRejestracyjny} onChange={handleInputChange} />
                            </div>
                            <div className="flex gap-4">
                                <div className="flex-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Marka</label><input name="marka" placeholder="np. Toyota" className={getInputClass('marka')} value={formData.marka} onChange={handleInputChange} /></div>
                                <div className="flex-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Model</label><input name="model" placeholder="np. Corolla" className={getInputClass('model')} value={formData.model} onChange={handleInputChange} /></div>
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Ubezpieczyciel</label>
                                <select name="ubezpieczyciel" className={getInputClass('ubezpieczyciel', 'select')} value={formData.ubezpieczyciel} onChange={handleInputChange}>
                                    <option value="">Wybierz z listy...</option>
                                    {TOWARZYSTWA.map(t => <option key={t} value={t}>{t}</option>)}
                                </select>
                            </div>
                            <div className="flex gap-4">
                                <div className="flex-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Nr polisy</label><input name="numerPolisy" placeholder="Numer polisy" className={getInputClass('numerPolisy')} value={formData.numerPolisy} onChange={handleInputChange} /></div>
                                <div className="flex-1"><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Data rozwiązania</label><input name="dataRozwiazania" type="date" className={getInputClass('dataRozwiazania', 'date')} value={formData.dataRozwiazania} onChange={handleInputChange} /></div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 md:col-span-2 space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                            <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Data podpisania</label><input name="dataPodpisania" type="date" className={getInputClass('dataPodpisania', 'date')} value={formData.dataPodpisania} onChange={handleInputChange} /></div>
                            <div><label className="block text-[10px] font-bold text-slate-400 uppercase mb-2 ml-1">Miejscowość podpisania</label><input name="miejscowoscWystawienia" placeholder="np. Szczecin" className={getInputClass('miejscowoscWystawienia')} value={formData.miejscowoscWystawienia} onChange={handleInputChange} /></div>
                        </div>
                    </section>
                        ...
                        {['28', '28a', '31'].map(a => (
                            <button key={a} onClick={() => setFormData({...formData, art: a})} className={`p-6 rounded-[2rem] border-2 flex items-center gap-4 transition-all ${formData.art === a ? 'bg-blue-50 border-[#0067b1] shadow-lg shadow-blue-100' : 'bg-white border-slate-100 hover:border-blue-200'}`}>
                                ...
                                <div className="text-left leading-tight">
                                    <span className="block font-black text-slate-900 text-sm">{a === '28' ? 'Standardowe' : a === '28a' ? 'Po wznowieniu' : 'Nabywcy'}</span>
                ...
                <button onClick={handleGenerateAndSave} className="w-full p-6 bg-[#0067b1] text-white rounded-[2rem] font-black shadow-2xl shadow-blue-200 flex items-center justify-center gap-4 hover:bg-blue-700 active:scale-[0.98] transition-all uppercase tracking-[0.2em] text-sm">
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center animate-in zoom-in-95 duration-300">
        ...
        <h2 className="text-3xl font-black text-slate-800" style={styles.headerFont}>Moduł {activeTab.toUpperCase()}</h2>
        <p className="text-slate-400 font-medium mt-3 max-w-sm">Ten moduł jest w trakcie wdrażania. Wkrótce pojawi się tutaj kompletna funkcjonalność.</p>
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-slate-900" style={styles.bodyFont}>
      {/* SIDEBAR */}
      <aside className={`bg-white border-r border-slate-100 transition-all duration-500 ${isSidebarOpen ? 'w-72' : 'w-24'} hidden md:flex flex-col z-30`}>
        <div className="p-8 flex items-center gap-4">
          ...
          {isSidebarOpen && (
            <div className="animate-in fade-in slide-in-from-left-4 duration-500">
              <span className="font-black text-2xl tracking-tighter text-[#0067b1]" style={styles.headerFont}>EGIDA</span>
        ...
        <div className="p-6 mt-auto">
          ...
          <button onClick={() => signOut(auth)} className={`w-full flex items-center gap-4 p-4 mt-4 text-rose-500 font-black text-[10px] uppercase tracking-[0.2em] hover:bg-rose-50 rounded-2xl transition-all ${!isSidebarOpen && 'justify-center'}`}>
            <LogOut size={20} />
            {isSidebarOpen && <span>Wyloguj system</span>}
          </button>
        </div>
      </aside>
      ...
      {/* MODALE I POWIADOMIENIA (Logika bez zmian) */}
      {showResetConfirm && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
              <div className="bg-white p-10 rounded-[3rem] w-full max-w-sm space-y-6 text-center shadow-2xl animate-in zoom-in-95 duration-300">
                  <div className="mx-auto text-rose-500 h-20 w-20 bg-rose-50 rounded-full flex items-center justify-center">
                      <RotateCcw size={40} />
                  </div>
                  <div>
                    <h3 className="font-black text-2xl text-slate-900" style={styles.headerFont}>Wyczyścić?</h3>
                    <p className="text-slate-400 font-medium text-sm mt-2">Wszystkie wprowadzone dane zostaną trwale usunięte z formularza.</p>
                  </div>
                  <div className="flex flex-col gap-3">
                      <button onClick={handleReset} className="w-full py-4 bg-rose-500 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg shadow-rose-100">Tak, wyczyść dane</button>
      ...
      {actionStatus === 'validation_error' && (
          <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 z-50 animate-in slide-in-from-bottom-12">
              ...
              <span className="font-black text-xs uppercase tracking-widest">Uzupełnij wymagane pola!</span>
          </div>
      )}
      ...
      {actionStatus === 'success' && (
          <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-emerald-500 text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 z-50 animate-in slide-in-from-bottom-12">
              ...
              <span className="font-black text-xs uppercase tracking-widest">Dokument wygenerowany pomyślnie!</span>
          </div>
      )}
    </div>
  );
